...
export const setNavigationDefinitions = (navigation: Array<any>) => {
  ...
  navigation.forEach((navItem) => {
    ...
    if (navItem.isRemote) {
      data.loadChildren = () =>
          loadRemoteModule(navItem.path, './Routes').then((m) => m.remoteRoutes)
    } else {

      const compName = (navItem.component.charAt(0).toUpperCase() + navItem.component.slice(1) + 'Component') as keyof  typeof Component
      data.loadComponent = () =>
