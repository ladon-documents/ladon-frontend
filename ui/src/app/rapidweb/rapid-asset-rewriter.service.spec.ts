...
describe('RapidAssetRewriterService', () => {
  let service: RapidAssetRewriterService;
  let htmlPolicy: RapidHtmlPolicyService;
  ...
  beforeEach(() => {
    ...
    service = TestBed.inject(RapidAssetRewriterService);
    htmlPolicy = TestBed.inject(RapidHtmlPolicyService);
  });
  ...
  it('rewrites script src before policy validation', () => {
    ...
    expect(html).toBe(
      '<script src="/admin/api/rest/v1/content/buckets/draco-rapids/documents?key=demo%2Fapp.js" defer=""></script>',
    );
  });
  ...
  it('rewrites stylesheet link href', () => {
    ...
    expect(html).toBe(
      '<link rel="preload stylesheet" href="/admin/api/rest/v1/content/buckets/draco-rapids/documents?key=demo%2Fstyle.css">',
    );
  });
  ...
  it('rewrites image and media src attributes', () => {
    ...
    expect(html).toContain(
      '<img src="/admin/api/rest/v1/content/buckets/draco-rapids/documents?key=demo%2Fimg%2Flogo.png">',
    );
    expect(html).toContain(
      '<video src="/admin/api/rest/v1/content/buckets/draco-rapids/documents?key=demo%2Fmedia%2Fclip.mp4"></video>',
    );
    expect(html).toContain(
      '<audio src="/admin/api/rest/v1/content/buckets/draco-rapids/documents?key=demo%2Fmedia%2Fsound.mp3"></audio>',
    );
    expect(html).toContain(
      '<source src="/admin/api/rest/v1/content/buckets/draco-rapids/documents?key=demo%2Fmedia%2Fclip.webm">',
    );
  });
  ...
  it('throws when a relative asset escapes the rapid folder', () => {
  ...
  it('parses inertly without executing inline scripts or event handlers', () => {
    (window as unknown as { rapidAssetRewriteExecuted?: boolean }).rapidAssetRewriteExecuted = false;

    const html = service.rewrite(
      '<img src="./logo.png" onerror="window.rapidAssetRewriteExecuted = true"><script>window.rapidAssetRewriteExecuted = true</script>',
      'demo/',
    );

    expect((window as unknown as { rapidAssetRewriteExecuted?: boolean }).rapidAssetRewriteExecuted).toBeFalse();
    expect(html).toContain('onerror="window.rapidAssetRewriteExecuted = true"');
    expect(html).toContain('<script>window.rapidAssetRewriteExecuted = true</script>');

    delete (window as unknown as { rapidAssetRewriteExecuted?: boolean }).rapidAssetRewriteExecuted;
  });
  ...
  it('rewrites relative script src values into same-origin external policy descriptors', () => {
    ...
    expect(plan.scripts[0].src).toBe(
      `${window.location.origin}/admin/api/rest/v1/content/buckets/draco-rapids/documents?key=demo%2Fapp.js`,
    );
