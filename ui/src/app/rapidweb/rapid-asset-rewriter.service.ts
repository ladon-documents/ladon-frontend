...
export class RapidAssetRewriterService {
  constructor(private readonly assetUrl: RapidAssetUrlService) {}
  ...
  private rewriteElementAttribute(element: Element, attribute: string, basePath: string, script: boolean): void {
    ...
    if (script && /[\\?#]/.test(value)) throw new Error('Rapid asset path is not allowed');
    if (!value || value.startsWith('#')) return;
    if (script && this.isExternalOrAbsoluteUrl(value)) throw new Error('Rapid asset path is not allowed');
