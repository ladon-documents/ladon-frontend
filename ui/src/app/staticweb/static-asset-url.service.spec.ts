...
describe('StaticAssetUrlService', () => {
  ...
  beforeEach(() => {
    ...
    service = TestBed.inject(StaticAssetUrlService);
  });
  ...
  it('encodes keys as query parameter values', () => {
    expect(service.buildAssetUrl('demo/', './assets/main file.css')).toBe(
      '/admin/api/rest/v1/content/buckets/draco-statics/documents?key=demo%2Fassets%2Fmain%20file.css',
    );
  });
  ...
  it('documents the browser asset endpoint that must be verified before enabling tag rewriting', () => {
    expect(StaticAssetUrlService.documentEndpoint).toBe('/admin/api/rest/v1/content/buckets/draco-statics/documents');
    expect(service.buildAssetUrl('demo/', './style.css')).toBe(
      `${StaticAssetUrlService.documentEndpoint}?key=demo%2Fstyle.css`,
    );
  });
});
