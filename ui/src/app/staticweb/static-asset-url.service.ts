...
export class StaticAssetUrlService {
  ...
  private normalizeBasePath(basePath: string): string {
    ...
    if (!decoded) throw new Error('Static asset path is not allowed');
    if (decoded.startsWith('/')) throw new Error('Static asset path is not allowed');
    if (this.containsTraversalSegment(decoded)) throw new Error('Static asset path is not allowed');

    const normalized = this.normalizePathParts(decoded.split('/'));
    if (!normalized) throw new Error('Static asset path is not allowed');
  ...
  private decodeSafePath(path: string): string {
    if (this.hasProtocolSyntax(path)) throw new Error('Static asset path is not allowed');
    if (/[\\?#]/.test(path)) throw new Error('Static asset path is not allowed');
    if (this.containsEncodedSeparator(path)) throw new Error('Static asset path is not allowed');

    let decoded: string;
    try {
      decoded = decodeURIComponent(path);
    } catch {
      throw new Error('Static asset path is not allowed');
    }

    if (this.hasProtocolSyntax(decoded)) throw new Error('Static asset path is not allowed');
    if (/[\\?#]/.test(decoded)) throw new Error('Static asset path is not allowed');
    if (this.containsEncodedSeparator(decoded)) throw new Error('Static asset path is not allowed');
    if (/%[0-9a-f]{2}/i.test(decoded)) throw new Error('Static asset path is not allowed');
  ...
  private normalizePathParts(parts: string[]): string {
    ...
    for (const part of parts) {
      ...
      if (part === '..') {
        if (normalized.length === 0) throw new Error('Static asset path is not allowed');
        normalized.pop();
      } else {
