...
export const BucketsStore = signalStore(
  ...
  withMethods(
    (store, bucketsService = inject(BucketsService)) => {
      const methods = {
        resetBucketsStore() {
        ...
        setSearchTerm: (searchTerm: string) => {
          const { filteredBuckets, paginatedBuckets, paginationState } =
            methods.applyFiltersAndPagination(
              store.allBuckets(),
              searchTerm,
        ...
        applyFilters: (buckets: BucketUiItemModel[], searchTerm: string, showFavoritesOnly: boolean): BucketUiItemModel[] => {
          let filtered = [...buckets];
          if (searchTerm) {
            const term = searchTerm.toLowerCase();
            filtered = filtered.filter(bucket =>
              bucket.id?.toLowerCase().includes(term)
            );
          }
